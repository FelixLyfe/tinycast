...
struct ShortcutRecorderPopover: View {
    ...
    private var state: State {
        ...
        let held = KeyShortcut.modifierSymbols(from: capture.heldModifiers)
        guard held.isEmpty else {
            return State(caps: held, label: String(localized: "Add a key"))
        }
        return State(
            caps: ["⌥", "A"],
            label: String(localized: "Type a shortcut"), isExample: true)
    }
}
