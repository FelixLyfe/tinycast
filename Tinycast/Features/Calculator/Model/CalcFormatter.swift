...
enum CalcFormatter {
    ...
    static func compoundFeetInches(_ feet: Double) -> String {
        ...
        let feetValue = sign + display(wholeFeet)
        let feetPart =
            wholeFeet == 0
            ? ""
            : wholeFeet == 1
                ? String(localized: "\(feetValue) foot")
                : String(localized: "\(feetValue) feet")
        let inchText = display(inches)
        let inchPart =
            inchText == "1"
            ? String(localized: "\(inchText) inch") : String(localized: "\(inchText) inches")
